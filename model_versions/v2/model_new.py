...
class NIT_Registration(NIT):
    ...
    def encode(self, pts_padded, pts_length, ref_idx):
        ...
        # print(self.xy_xz_yz_xyz_weight)
        # breakpoint()
    ...
    def forward(self, pts, match_dict=None, ref_idx=0, mode='train'):
        ...
        layers_sim_m = list(torch.stack([torch.bmm(mov_emb, ref_emb.transpose(dim0=1, dim1=2)) for mov_emb,ref_emb in zip(mov_emb_list, ref_emb_list)]))
        # the outlier of mov node
        ...
        if (mode == 'train') or (mode == 'all'):
            ...
            # breakpoint()
            layers_sim_m.append(layers_sim_m[5]+layers_sim_m[6]+layers_sim_m[8]+layers_sim_m[10])
