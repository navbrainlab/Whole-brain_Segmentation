...
class NIT_Registration(nn.Module):
    ...
    def __init__(self, input_dim, n_hidden, n_layer=6, cuda=True, p_rotate=False, feat_trans=False,
        ...
        self.xyz_ptf = nn.Sequential(
            nn.Conv1d(4, n_hidden, 1),
            nn.BatchNorm1d(n_hidden)
        )
        self.temp_4_align = np.load('Data/test_tracking/real_jeff_all/real_808.npy')
    ...
    def get_axis_dist(self, pts):
        new_pt_batch = []
        for pts in pts:
            pts = src_utils.get_angle_aligned_points(pts,self.temp_4_align)
            new_pt_batch.append(pts)

        max_index = torch.tensor([np.argmax(i[:,1]) for i in new_pt_batch])
        return max_index



    def fit_line(self, datas):
        a_ = []
        b_ = []
        c_ = []
        for data in datas:
        ...
        return torch.stack((torch.tensor(a_, device=self.device),
                torch.tensor(b_, device=self.device),
                torch.tensor(c_, device=self.device),),dim=1)
    ...
    def forward(self, pts, match_dict=None, ref_idx=0, mode='train'):
        ...
        anchor_index = self.get_axis_dist(pts)
        # fit line
        abc = self.fit_line(pts)
        # Compute sentence lengths
        pts_lengths = [len(s) for s in pts]
        # pts_padded should be of dimension (
        pts_padded = self.to_input_tensor(pts)
        z_online = torch.stack([torch.sum(i[:,:2]*abc_[:2],dim=1) + abc_[2] for i,abc_ in zip(pts_padded, abc)]).type(torch.FloatTensor).to(self.device)


        xy_online = pts_padded[torch.arange(len(pts_lengths)), anchor_index][:,:2]
        anchor_z = z_online[torch.arange(len(pts_lengths)), anchor_index]
        xyz_online = torch.concatenate([xy_online, anchor_z[:,None]],dim=1)
        xyz_dist = torch.sqrt(torch.sum(torch.square(pts_padded - xyz_online[:,None,:]),dim=-1))
        cat_dist_feat = torch.concatenate([pts_padded, xyz_dist[:,:,None]], dim=-1)

        # mov_emb, ref_emb = self.encode(pts_padded, pts_lengths, ref_idx)


        pts_encode, encoded_pair_contrastive = self.encode(cat_dist_feat, pts_lengths, ref_idx)
        ...
        if (mode == 'train') or (mode == 'all'):
            ...
            for i_w in range(batch_sz):
                ...
                if len(match) > 0:
                    ...
                    # dist_weight = [self.get_dist_weight(p_m[i_w], pts_padded[i_w], match_mov, match_ref) for p_m in p_m_exp]
                    # # along x axis,
                    # log_p = torch.stack([p[i_w, match_mov, match_ref]*weight for p,weight in zip(p_m_exp,dist_weight)])## 每个test neuron与真实对应temp neuron的匹配概率
                    # log_p_neg = torch.stack([p[i_w, match_mov, :]*weight[:,None] for p,weight in zip(p_m_exp,dist_weight)])
                    # loss += (log_p_neg.sum() - 2*log_p.sum())/torch.prod(torch.tensor(log_p.shape))

                    # # -----------------------------along y axis
                    # log_p = torch.stack([p[i_w, match_mov, match_ref]*weight for p,weight in zip(p_m_exp_alongy,dist_weight)])## 每个test neuron与真实对应temp neuron的匹配概率
                    # log_p_neg = torch.stack([p[i_w, :, match_ref]*weight[None,:] for p,weight in zip(p_m_exp_alongy,dist_weight)])
                    # loss += (log_p_neg.sum() - 2*log_p.sum())/torch.prod(torch.tensor(log_p.shape))
