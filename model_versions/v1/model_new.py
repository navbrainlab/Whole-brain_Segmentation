...
class NIT(nn.Module):
    ...
    def __init__(self, input_dim, n_hidden, n_layer=6, cuda=True, p_rotate=False, feat_trans=False):
        ...
        self.enc_l = nn.TransformerEncoderLayer(d_model=n_hidden, nhead=8, norm_first=True, dim_feedforward=2048)
        self.encoder_model = TransformerEncoder(self.enc_l, self.encoder_layer_num, norm = encoder_norm)

        # self.encoder_model = nn.Identity()
        self.dec_l = nn.TransformerDecoderLayer(d_model=n_hidden, nhead=8, norm_first=True, dim_feedforward=2048)
...
class NIT_Registration(NIT):
    ...
    def pointnet_normalization(self, points):
        # points: (B, N, C) - B: batch size, N: number of points, C: number of features
    ...
    def encode(self, pts_padded, pts_length, ref_idx):
        ...
        pts_encode = self.encoder_model(pts_proj.transpose(dim0=0, dim1=1))
        ...
        mem_mask = None
        if self.training and self.drop_neuron_ratio:
            mem_mask = self.generate_mem_mask(num_rows=tgt_input.shape[1], num_cols=ref_pts_proj.shape[0], r=self.drop_neuron_ratio, device=tgt_mask.device)
        ...
        # ref_pts_proj = torch.mean(ref_pts_proj.view([pts_length[ref_idx],-1]+list(ref_pts_proj.size()[-2:])),dim=1)
    ...
    def forward(self, pts, match_dict=None, ref_idx=0, mode='train'):
        ...
        #     ref_emb_list = [pts_encode[i][:, :pts_lengths[ref_idx], :] for i in range(len(pts_encode))]
        #     mov_emb_list = [pts_encode[i][:, pts_lengths[ref_idx]:, :] for i in range(len(pts_encode))]
        #     sim_m = torch.bmm(mov_emb_list[-1], ref_emb_list[-1].transpose(dim0=1, dim1=2))
        #     # the outlier of mov node
        #     mov_outlier = self.fc_outlier(mov_emb_list[-1])
        #     sim_m = torch.cat((sim_m, mov_outlier), dim=2)
        #     p_m = F.log_softmax(sim_m, dim=2)
        #     p_m_exp = F.softmax(sim_m, dim=2)
        ...
        if (mode == 'train') or (mode == 'all'):
            ...
            layers_sim_m.append(torch.mean(torch.stack(layers_sim_m),dim=0))
