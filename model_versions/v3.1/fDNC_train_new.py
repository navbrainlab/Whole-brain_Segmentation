...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr_decay", default=0.6, type=float, help="learning rate decay")
    ...
    parser.add_argument("--model_idx", default=629, type=int)
    ...
    parser.add_argument("--drop_neuron_ratio", default=0.3, type=float)
    tic = time.time()
    ...
    print('drop_neuron_ratio:{}\n'.format(args.drop_neuron_ratio))
    cuda = args.cuda


    file_path = f'Exp_logs/{args.model_idx}.txt'
    with open(file_path, 'w') as file:
        # 执行对文件的操作，例如写入内容
        file.write(f'{args.model_idx}\n')
    ...
    train_data_loader = DataLoader(train_data, shuffle=False, num_workers=8, collate_fn=train_data.custom_collate_fn)
    dev_data_loader = DataLoader(dev_data, shuffle=False, num_workers=8, collate_fn=dev_data.custom_collate_fn)
    ...
    model = NIT_Registration(input_dim=3, n_hidden=args.n_hidden, n_layer=args.n_layer, p_rotate=args.p_rotate,
                ...
                use_embedding=True,
                drop_neuron_ratio=args.drop_neuron_ratio)
    if args.use_pretrain:
        pretrain_path = 'model/nitReg_nh128_nl6_ft0_dataall_elam_0.1_718_drn0.0.bin'
    ...
    model_name = '{}_nh{}_nl{}_ft{}_data{}_elam_{}_{}_drn{}.bin'.format(args.model_name, args.n_hidden, args.n_layer,
                                                                  args.f_trans, args.data_mode, args.lamb_entropy,
                                                                  args.model_idx, args.drop_neuron_ratio)
    ...
    while True:
        ...
        for batch_idx, data_batch in enumerate(train_data_loader):
            ...
            loss = batch_loss['loss'] / batch_loss['num'] + args.lamb_entropy * batch_loss['loss_entropy'] / batch_loss['num_unlabel']
