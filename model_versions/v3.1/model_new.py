...
class NIT_Registration(nn.Module):
    ...
    def for_encode(self, pts_proj, pts_length, mask, ref_idx):
        pts_encode = self.encoder_model(pts_proj.transpose(dim0=0, dim1=1),src_key_padding_mask=mask)[5:6]

        ref_pts_proj = torch.concatenate([pts_enc[:pts_length[ref_idx], ref_idx:ref_idx+1, :] for pts_enc in pts_encode],dim=0)
        ...
        decoded_to_return = [torch.concatenate([ref_pts_emb,pts_d],dim=0).transpose(1,0) for ref_pts_emb, pts_d in zip(ref_pts_proj,pts_decoded)]
